...
import DeviceTile from '../components/Smart/DeviceTile';
import Screen from '../components/Screen';
import useColors from '../hooks/useColors';
import Indicator1 from '../components/Smart/indicator1/CurtainSwitch';
import Indicator2 from '../components/Smart/indicator2/CurtainSwitch';
...
const INITIAL_DEVICE_CATEGORIES = [
  {
        ...
        items: [
          ...
          lan: {
            device_id: '0c110500755b_71_2',
            ability_id: 'light.0c110500755b_71_2',
            commandPair: { on: 'turn_on', off: 'turn_off' }
          },
          wan: {
...
export default function SmartScreen() {
  ...
  const categories = [
    { category: 'All Devices', items: allDevices },
    ...deviceCategories,
  ];
...
const LAN_HEADERS = {
  ...
  "Authorization": "Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJpc3MiOiIwMTFkYWU2MWFhODI0NTg1YWRiZGMxMzUxZTE0NWZiZCIsImlhdCI6MTc1NTg2MzUyNywiZXhwIjoxNzg3Mzk5NTI3fQ.FCHGEp9ExyoFag720r3TOtztUQXj19pGLQDAPMqg5DM"
};


const controlDevice = async (deviceId, abilityId, command, attribute) => {
  if (selectedOption === 'LAN') {
    // LAN API call (direct to device)
    const lanApiUrl = `http://192.168.1.108/api/v1.0/device`;
    const body = {
      ...
      param: {
        ...
        action : command, // LAN expects 'action' key
        ...(attribute ? { attribute } : {}),
      },
    };
    ...
    // WAN API call (to backend; backend handles auth headers)
        ...
        devices: [
          {
            device_id: deviceId,
            ability_id: abilityId,
            control: command, // WAN expects 'control' key
            ...(attribute ? { attribute } : {}),
          },
        ],
      },
    ...
    try {
      const response = await fetch(apiUrl, {
        ...
        headers: {
          "Content-Type": "application/json",
          // No Authorization header needed here; backend adds it
        },
        body: JSON.stringify(body),
      });
...
const deviceStatus = async (deviceId ) => {
  if (selectedOption === 'LAN') {
    console.log('LAN device status:', deviceId);
      const lanApiUrl = `http://192.168.1.108/api/v1.0/device`;
      const body = {
...
useEffect(() => {
  // Only fetch when modal is opening and a device is selected
  if (modalVisible && selectedDevice) {
    ...
    // Now fetch device status
    const fetchStatus = async () => {
      const apiUrl = selectedOption === 'LAN'
        ? 'http://10.255.254.20/api/v1.0/device'
        : 'http://10.255.254.28:8000/device_status/';
      const headers = selectedOption === 'LAN'
        ? LAN_HEADERS
        : { "Content-Type": "application/json" };
      const body = {
        command: 'get_device_info',
...
const handleToggle = (device, newControl) => {
  ...
  controlDevice(dev.device_id, dev.ability_id, command);
  deviceStatus(dev.device_id);
};
...
const handleSetPosition = (device, position) => {
  ...
  // Command for set position: "turn_on" for LAN, "open" for WAN
  const command = mode === 'lan' ? dev.commandPair['on'] : dev.commandPair['on'];
   const attribute = mode === 'lan'
    ? { position }
    : { position_percent: position };
  controlDevice(dev.device_id, dev.ability_id, command, attribute);
};
...
const handleSetTemperature = (device, temperature) => {
  ...
  // Set correct attribute key for each mode
  const attribute = mode === 'lan'
...
const handleSetBrightnes = (device, Brightness) => {
  const mode = selectedOption.toLowerCase();
  const dev = device[mode];
  if (!dev || !dev.commandPair) {
    Alert.alert('Not supported', `This device cannot be controlled via ${selectedOption}.`);
        ...
        {filteredDevices
          .reduce((result, device, index) => {
            if (index % 2 === 0) {
              ...
              result[result.length - 1].push(device);
            }
            return result;
          }, [])
          .map((row, rowIndex) => (
